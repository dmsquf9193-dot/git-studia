...
SELECT *
FROM (SELECT first_name, avg(salary) AS avg_salary
      FROM employees e
      JOIN salaries s ON e.emp_no = s.emp_no
      GROUP BY first_name)g
      WHERE avg_salary >= 80000;

-- Q5. 사원의 이름(first_name)과 급여(salary)를 급여가 높은 순으로 조회하세요.
SELECT *
FROM (SELECT e.first_name, max(s.salary)
      FROM employees e
      JOIN salaries s ON e.emp_no  = s.emp_no
      GROUP BY e.emp_no ) a; 
...
SELECT *
FROM (SELECT d.dept_name, count(de.dept_no)
FROM departments d 
JOIN dept_emp de  ON d.dept_no = de.dept_no 
GROUP BY d.dept_no)l;


-- Q7. 부서별 사원 수가 20000명 이상인 부서의 이름(dept_name)과 사원수(count)만 조회하세요.
SELECT * 
FROM (SELECT d.dept_name, count(de.dept_no) AS avg_dept_no
      FROM departments d 
      JOIN dept_emp de ON d.dept_no = de.dept_no 
      GROUP BY d.dept_name)s
      WHERE s.avg_dept_no >= 20000;
